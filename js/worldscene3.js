class WorldScene3 extends Phaser.Scene {
  constructor() {
    super({ key: "WorldScene3" });
  }
  ...
  preload() {
    ...
    this.load.tilemapTiledJSON("map3", "assets/trabp3.json");
  ...
  create() {
    ...
    var tileset3 = map3.addTilesetImage("platformPack_tilesheet", "tiles");

    //adiciona os arbustos
    map3.createStaticLayer("adereços", tileset3, 0, 0);
    ...
    var platforms = map3.createStaticLayer("Plataformas", tileset3, 0, 0);
    ...
    var turbinaObjects = map3.getObjectLayer("turbinas")["objects"];
    ...
    //Adicionar as bolas de fogo no jogo
    bolas = this.physics.add.group({
      allowGravity: false,
      immovable: true,
    });
    var bolaObjects = map3.getObjectLayer("aguas")["objects"];
    var i = 0;
    ...
    var lavaObjects = map3.getObjectLayer("aguas")["objects"];
  ...
  makeInimigos() {
    ...
    while (nInimigos != 3) {
      ...
      var x = Phaser.Math.Between(400, 2000);

      var inimigo = inimigos.create(x, 16, "inimigo");
      inimigo.setBounce(1);
      inimigo.setCollideWorldBounds(true);
      inimigo.setVelocity(Phaser.Math.Between(-300, 300), 20);
      inimigo.allowGravity = false;
      inimigo.setScale(4, 4);
    }
  }
